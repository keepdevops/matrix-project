...
| **2c** | **Paged KV — spike → gate.** `MATRIX_PAGED_KV_ENABLED`: Metal-backed pages, prefix reuse + eviction. | **High / uncertain** | Spike + probe first: measured allocation/eviction; **off by default byte-identical**. May land as spike-only if perf doesn't justify. |
