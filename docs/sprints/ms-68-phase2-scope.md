...
| **2d** ❌ | **Flash attention — SPIKE DONE → NO-GO** ([findings](./ms-68-2d-flash-attention-spike.md)). | — | **NO-GO**: MLX's `mx.fast.scaled_dot_product_attention` (fused/flash) is already used by `mlx_lm` on every token — nothing for our layer to wrap. `flash_attention_wrapper` stub deleted (with `paged_kv_cache` per 2c). |
