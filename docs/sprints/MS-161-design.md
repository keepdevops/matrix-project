...
**Status:** ✅ Phases A, B, C, D complete (2026-06-03). In-process MLX submit **and streaming** are opt-in behind MATRIX_MLX_INPROC + per-agent `dispatch:inproc`.
...
| **C — streaming + sequential modes** | ✅ **DONE (2026-06-03).** `MlxModelRegistry::generate_stream` drives `mlx_lm.stream_generate` from C++ via `PyIter_Next`, emitting one SSE `token` event per chunk through the same serialized lane. `/api/mlx/stream` `run_one` routes `dispatch=="inproc"` agents in-process (flat + pipeline); the cascade synthesizer step routes in-process too. | **Verified e2e (clean env, isolated port):** same coordinator process — `submit` cold 5.1s (load+gen, coherent answer), then `stream` 0.5s with **32 token events** streamed in-process (`"1"`, `","`, …), **0** HTTP-fallback / inproc-error events; `/api/mlx/pressure` reports `resident_count=1` (model reused, not reloaded). Registry probe independently streamed 27 chunks with clean refcounting. Flag-off build keeps flat-mode fan-out on HTTP unchanged. |
| **D — ship gate** | ✅ **DONE (2026-06-03); caveats closed (2026-06-03).** `/api/mlx/pressure` surfaces `resident_models` + `resident_count` (under flag); operator doc `docs/model-management.md`. Soak script `scripts/soak_coordinator.py`; ASan build flag `MATRIX_SANITIZE=address` + suppressions `scripts/asan_python.supp`. | Soak (routing path, sequential pipeline): **6704 submits, 0 fail, FDs flat at 4 (no descriptor leak), no crash** — process ended only on external SIGTERM, not a fault. RSS grew 40→85 MB from persistent history/session accumulation (by design, not an FD/connection leak). **ASan: 4/4 backend-routing unit tests clean** (use-after-free / overflow); the shipped coordinator is pure C++ so needs no CPython suppressions — LeakSanitizer is N/A on macOS arm64, and the `.supp` file covers the optional in-process build. Tests: `mlx_coordinator` **149+ passed / 0 failed**; `test_streaming` 8/8 (incl. Phase C routing). |
