...
void register_proxy_routes_local(httplib::Server& svr, const std::string& proj_root) {
    ...
    svr.Get("/api/swarm-config", [&proj_root](const httplib::Request&, httplib::Response& res) {
        apply_cors(res);
        try {
    ...
    svr.Post("/api/configure", [&proj_root](const httplib::Request& req, httplib::Response& res) {
        apply_cors(res);
        try {
    ...
    svr.Get("/api/configure/status", [](const httplib::Request&, httplib::Response& res) {
        apply_cors(res);
    ...
    svr.Get("/api/logs", [&proj_root](const httplib::Request& req, httplib::Response& res) {
        apply_cors(res);
    ...
    svr.Get("/api/swarm/status", [](const httplib::Request&, httplib::Response& res) {
        apply_cors(res);
        try {
    ...
    svr.Post("/api/inference/vllm/start", [&proj_root](const httplib::Request&, httplib::Response& res) {
        apply_cors(res);
...
void register_proxy_coordinator_forward(httplib::Server& svr) {
    auto fwd = [](const httplib::Request& req, httplib::Response& res) {
        res.set_header("Access-Control-Allow-Origin", "*");
        httplib::Client coord("127.0.0.1", g_env.coordinator_port);
        coord.set_connection_timeout(5);
