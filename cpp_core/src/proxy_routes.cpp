...
void register_proxy_routes(httplib::Server& svr, const std::string& proj_root) {
    ...
    // Chunks arrive from the Python coord and are forwarded to the browser as they land.
            ...
            [req_body, port](size_t, httplib::DataSink& sink) {
                ...
                py.Post("/api/orchestrate/stream", hdrs,
                    ...
                    [&req_body](size_t off, size_t len, httplib::DataSink& ds) {
                        size_t n = std::min(len, req_body.size() > off ? req_body.size() - off : (size_t)0);
    ...
    // Blocking JSON fallback for Python orchestration modes (used when streaming is not needed).
