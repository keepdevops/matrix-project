...
if ! $NODE_OK; then
    if [[ "$ARCH" == "arm64" || "$ARCH" == "x86_64" ]] && [[ "$(uname -s)" == "Darwin" ]]; then
...
# ─── 5. Build Coordinator & C++ proxy ─────────────────────────────────────────
banner "5/8  Build Coordinator & C++ proxy (C++17)"

cd "$ROOT"
CXX_CMD="${CXX:-c++}"
command -v "$CXX_CMD" &>/dev/null || CXX_CMD="${CXX:-g++}"
...
if [[ -f coordinator && coordinator -nt coordinator.cpp ]]; then
    ...
    ok "coordinator binary built: $ROOT/coordinator"
fi

if [[ -f proxy ]] && [[ proxy -nt proxy.cpp ]] && [[ proxy -nt proxy_configure.cpp ]] && [[ proxy -nt matrix_env.cpp ]]; then
    ok "proxy binary is up-to-date (skipping rebuild)"
else
    info "Compiling proxy (proxy.cpp, proxy_configure.cpp, matrix_env.cpp)..."
    $CXX_CMD -std=c++17 -O2 -o proxy proxy.cpp proxy_configure.cpp matrix_env.cpp -pthread
    ok "proxy binary built: $ROOT/proxy"
fi
...
if [[ "$ARCH" == "arm64" ]]; then
    ...
    if [[ "${MLX_CHOICE,,}" == "y" ]]; then
        info "Installing mlx-lm via conda (matrix-mlx environment)..."
        CONDA_CMD=""
        for _c in "$(command -v mamba 2>/dev/null)" "$(command -v conda 2>/dev/null)" \
                  "$HOME/miniforge3/bin/conda" "$HOME/miniconda3/bin/conda" "$HOME/anaconda3/bin/conda"; do
            [[ -x "$_c" ]] && { CONDA_CMD="$_c"; break; }
        done
        if [[ -n "$CONDA_CMD" ]]; then
            "$CONDA_CMD" create -n matrix-mlx python=3.11 -y --quiet
            _mlx_prefix=$("$CONDA_CMD" env list 2>/dev/null | awk '/matrix-mlx/{print $NF}')
            "$_mlx_prefix/bin/pip" install mlx-lm --quiet
            ok "MLX environment ready at $_mlx_prefix"
            ok "Select 'MLX' engine in UI — uses conda env matrix-mlx"
        else
            warn "conda not found — install Miniforge3 first, then re-run"
            info "  curl -fsSL https://github.com/conda-forge/miniforge/releases/latest/download/Miniforge3-MacOSX-arm64.sh | bash"
        fi
    else
...
check_item "Node.js (≥18)"         "node --version &>/dev/null"
check_item "node_modules"          "[[ -d '$ROOT/node_modules' ]]"
check_item "coordinator binary"    "[[ -x '$ROOT/coordinator' ]]"
check_item "proxy binary (C++)"    "[[ -x '$ROOT/proxy' ]]"
...
if (( GGUF_FINAL == 0 && MLX_FINAL == 0 )); then
    ...
    echo    "      or re-run: bash scripts/install.sh"
    echo
fi
echo    "  Docs:"
