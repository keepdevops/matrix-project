...
function reducer(state, action) {
  switch (action.type) {
    ...
    case 'MOVE_FREE_PANEL':
      return {
        ...state,
        freePanels: state.freePanels.map(p =>
    ...
    case 'RESIZE_FREE_PANEL':
      return {
        ...state,
        freePanels: state.freePanels.map(p =>
          p.id === action.id ? { ...p, w: Math.max(120, action.w), h: Math.max(80, action.h) } : p
        ),
      };
    case 'BRING_FORWARD':
      return {
    ...
    case 'SEND_BACK':
      return {
        ...state,
        freePanels: state.freePanels.map(p =>
          p.id === action.id ? { ...p, z: Math.max(1, p.z - 1) } : p
        ),
      };
