...
export default function ModeRosterPanel() {
  ...
  const [enginePolicy, setEnginePolicy] = useState('mlx-strict');
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setEnginePolicy(typeof data.engine_policy === 'string' ? data.engine_policy : 'mlx-strict');
  ...
  const save = async () => {
    ...
    try {
      ...
      if (activeTab === 'router') opts.engine_policy = enginePolicy;
      ...
      {activeTab === 'router' && (
        <div style={{ marginTop: '0.5rem', fontSize: '0.85rem',
          ...
          <select
            ...
            style={{ padding: '0.15rem 0.3rem' }}
          >
            <option value="mlx-strict">mlx-strict (default)</option>
            <option value="mlx-preferred">mlx-preferred</option>
            <option value="llama-only">llama-only</option>
            <option value="mixed">mixed</option>
          </select>
          <span style={{ opacity: 0.6, fontSize: '0.75rem' }}>
            controls whether router auto-prefers MLX agents
          </span>
        </div>
      )}
