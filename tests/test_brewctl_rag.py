"""Tests for RAG helper functions in scripts/brewctl.
...
# brewctl has no .py extension; load it by file path.
...
with patch.dict(sys.modules, {
    ...
    _loader = SourceFileLoader("brewctl", str(SCRIPTS / "brewctl"))
    _spec = importlib.util.spec_from_loader("brewctl", _loader)
    brewctl = importlib.util.module_from_spec(_spec)
    sys.modules["brewctl"] = brewctl
    _loader.exec_module(brewctl)
...
def test_pick_embedder_hash_returns_hash_instance():
    fake_hash = MagicMock()
    with patch.object(brewctl, "HashEmbedder", return_value=fake_hash):
        result = brewctl._pick_embedder("hash")
    assert result is fake_hash


def test_pick_embedder_mlx_returns_mlx_instance():
    fake_mlx = MagicMock()
    with patch.object(brewctl, "MLXEmbedder", return_value=fake_mlx):
        result = brewctl._pick_embedder("mlx")
    assert result is fake_mlx


def test_pick_embedder_unknown_raises_value_error():
    with pytest.raises(ValueError, match="unknown embedder"):
        brewctl._pick_embedder("openai")
...
def test_index_includes_python_files(tmp_path):
    ...
    with patch.object(brewctl, "chunk_text", return_value=[chunk]):
        _run(brewctl._index(tmp_path, embedder, store))
...
def test_index_skips_node_modules(tmp_path):
    ...
    with patch.object(brewctl, "chunk_text", return_value=[]) as mock_chunk:
        _run(brewctl._index(tmp_path, embedder, store))
...
def test_index_continues_on_read_error(tmp_path):
    ...
    with patch.object(brewctl, "chunk_text", return_value=[]), \
         patch("pathlib.Path.read_text", selective_read):
        # Must not raise even though one file fails to read.
        total = _run(brewctl._index(tmp_path, embedder, store))
...
def test_query_prints_hits(capsys):
    ...
    with patch.object(brewctl, "retrieve", mock_retrieve):
        _run(brewctl._query("hello", 3, embedder, store))
...
def test_query_truncates_long_content(capsys):
    ...
    with patch.object(brewctl, "retrieve", AsyncMock(return_value=[hit])):
        _run(brewctl._query("a", 1, MagicMock(), MagicMock()))
...
def test_query_no_hits_prints_nothing(capsys):
    with patch.object(brewctl, "retrieve", AsyncMock(return_value=[])):
        _run(brewctl._query("nothing", 5, MagicMock(), MagicMock()))
